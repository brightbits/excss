  ...
  describe ".accept" do
    context "doesn't match [" do
      it "returns an unchanged state and no token" do
        {new_state, token} = ExCss.Lexer.Consumers.OpenSquare.accept(TestHelper.state_for("[abc]", 0))
    ...
    context "matches ]" do
      it "returns an open square advances 1 char" do
        {new_state, token} = ExCss.Lexer.Consumers.OpenSquare.accept(TestHelper.state_for("[abc]"))
        expect(new_state.pos) |> to_eq(0)
        expect(token) |> to_eq({:open_square, {}})
      end
    end
  end
end
