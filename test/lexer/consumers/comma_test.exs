  ...
  describe ".accept" do
    context "doesn't match ," do
      it "returns an unchanged state and no token" do
        {new_state, token} = ExCss.Lexer.Consumers.Comma.accept(State.new("1,2,3", -1))
    ...
    context "matches ," do
      it "returns a comma token advances 1 char" do
        {new_state, token} = ExCss.Lexer.Consumers.Comma.accept(State.new("1,2,3", 0))
        expect(new_state.i) |> to_eq(1)
        expect(token) |> to_eq(%Tokens.Comma{})
      end
    end
  end
end
