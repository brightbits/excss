  ...
  describe ".accept" do
    context "doesn't match :" do
      it "returns an unchanged state and no token" do
        {new_state, token} = ExCss.Lexer.Consumers.Colon.accept(TestHelper.state_for("1:2:3", -1))
    ...
    context "matches :" do
      it "returns a semicolon token advances 1 char" do
        {new_state, token} = ExCss.Lexer.Consumers.Colon.accept(TestHelper.state_for("1:2:3", 0))
        expect(new_state.pos) |> to_eq(1)
        expect(token) |> to_eq({:colon, {}})
      end
    end
  end
end
