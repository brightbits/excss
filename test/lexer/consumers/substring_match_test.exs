defmodule ConsumersSubstringMatchTest do
  ...
  describe ".accept" do
    context "not starting with a $" do
      it "returns no token and doesn't advance the state" do
        {new_state, token} = ExCss.Lexer.Consumers.SubstringMatch.accept(TestHelper.state_for("abc"))
    ...
    context "starts with a *" do
      ...
      context "followed by a =" do
        it "returns a substring match and advances 2 chars" do
          {new_state, token} = ExCss.Lexer.Consumers.SubstringMatch.accept(TestHelper.state_for("*=abc"))
          expect(new_state.pos) |> to_eq(1)
          expect(token) |> to_eq({:substring_match, {}})
        end
      end
    end
  end
