  ...
  describe ".accept" do
    context "doesn't match ]" do
      it "returns an unchanged state and no token" do
        {new_state, token} = ExCss.Lexer.Consumers.CloseSquare.accept(TestHelper.state_for("[abc]", 2))
    ...
    context "matches )" do
      it "returns an close square advances 1 char" do
        {new_state, token} = ExCss.Lexer.Consumers.CloseSquare.accept(TestHelper.state_for("[abc]", 3))
        expect(new_state.pos) |> to_eq(4)
        expect(token) |> to_eq({:close_square, {}})
      end
    end
  end
end
