  ...
  describe ".accept" do
    context "not opening a string" do
      it "returns no token and doesn't advance the state" do
        {new_state, token} = ExCss.Lexer.Consumers.String.accept(TestHelper.state_for("abc"))
    ...
    context "is opening a string" do
      it "returns a string token and advances the state until the end of the string" do
        {new_state, token} = ExCss.Lexer.Consumers.String.accept(TestHelper.state_for("aaa\"this is a test\" test", 2))
      ...
      context "the string doesn't end before the EOF" do
        it "returns a string token and advances the state until the end of the string" do
          {new_state, token} = ExCss.Lexer.Consumers.String.accept(TestHelper.state_for("aaa\"this is a test", 2))
          expect(new_state.pos) |> to_eq(18)
          expect(token) |> to_eq({:string, {"this is a test"}})
        end
      end
      ...
      context "the string has a new line that isn't escaped" do
        it "returns a bad string token and adds a warning" do
          {new_state, token} = ExCss.Lexer.Consumers.String.accept(TestHelper.state_for("aaa\"this is a test\ncats\"", 2))
      ...
      context "the string has an escaped quote" do
        it "returns a string token" do
          {new_state, token} = ExCss.Lexer.Consumers.String.accept(TestHelper.state_for("aaa \"this is a \\\"test\" :)", 3))
          expect(token) |> to_eq({:string, {"this is a \"test"}})
      ...
      context "the string has an escaped hex" do
        it "returns a string token" do
          {new_state, token} = ExCss.Lexer.Consumers.String.accept(TestHelper.state_for("aaa \"this is a \\c582 :)\"", 3))
