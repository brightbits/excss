  ...
  describe ".accept" do
    context "doesn't match a digit" do
      it "returns an unchanged state and no token" do
        {new_state, token} = ExCss.Lexer.Consumers.Digits.accept(TestHelper.state_for("a123"))
    ...
    context "matches a digit" do
      context "and the following looks like a number" do
        it "returns a number token"  do
          {new_state, token} = ExCss.Lexer.Consumers.Digits.accept(TestHelper.state_for("123.456 test"))
          expect(new_state.pos) |> to_eq(6)
          expect(token) |> to_eq({:number, {123.456, "123.456"}})
        end

        it "works with dimensions also" do
          {new_state, token} = ExCss.Lexer.Consumers.Digits.accept(TestHelper.state_for("123.456em test"))
