  ...
  describe ".accept" do
    context "doesn't match )" do
      it "returns an unchanged state and no token" do
        {new_state, token} = ExCss.Lexer.Consumers.CloseParenthesis.accept(State.new("(abc)", 2))
    ...
    context "matches )" do
      it "returns an close parenthesis advances 1 char" do
        {new_state, token} = ExCss.Lexer.Consumers.CloseParenthesis.accept(State.new("(abc)", 3))
        expect(new_state.i) |> to_eq(4)
        expect(token) |> to_eq(%Tokens.CloseParenthesis{})
      end
    end
  end
end
