defmodule ExCss.Parser.Nodes.SimpleBlock do
  ...
  defp consume_a_simple_block(state, ending_token_type, simple_block) do
    ...
    if State.currently?(state, [T.EndOfFile, ending_token_type]) do
      {State.consume(state), %{simple_block | value: Enum.reverse(simple_block.value)}}
    else
      {state, component_value} =
        state
        |> State.consume_component_value

      consume_a_simple_block(state, ending_token_type, %{simple_block | value: [component_value] ++ simple_block.value})
    end
  end
end
